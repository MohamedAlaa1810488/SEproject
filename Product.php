...
class Product extends database implements operation{
    ...
    public function getAllData(){
        $query = "SELECT `products`.* FROM `products` ORDER BY `products`.`id` DESC";
    ...
    public function getTopRated(){
        $query = "SELECT `product_reviews`.* FROM `product_reviews`  ORDER BY  `product_reviews`.`product_count_reviews` DESC ,`product_reviews`.`product_avg_rate`  DESC LIMIT 4";
        return $this->runDQL($query);
    }

    public function getBestSeller(){
        $query = " SELECT `products_count`.* FROM `products_count` LIMIT 4";
        return $this->runDQL($query);
    }
